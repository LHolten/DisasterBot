...
def box_local_collision_location(local_point: np.ndarray,
                                 box_corner=HITBOX, box_offset=HITBOX_OFFSET):
    """point of contact in local coordinates"""
    return np.clip(local_point - box_offset, -box_corner, box_corner) + box_offset


def box_point_collision_location(point_loc, box_loc, box_rot_matrix,
                                 box_corner=HITBOX, box_offset=HITBOX_OFFSET):
...
def box_ball_location_on_collision(ball_loc, box_loc, box_rot_matrix,
                                   box_corner=HITBOX, box_offset=HITBOX_OFFSET, ball_radius=92):
    ...
    box_local_location_on_collision = ball_local_loc + normalize_batch(
        hit_local_loc - ball_local_loc) * ball_radius - hit_local_loc
    return box_rot_matrix.dot(box_local_location_on_collision) + box_loc


def box_ball_collision_distance(ball_loc, box_loc, box_rot_matrix,
                                box_corner=HITBOX, box_offset=HITBOX_OFFSET, ball_radius=92):
