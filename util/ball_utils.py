...
def get_high_ball_intercept_state(game_data, box_location=None):
    ball_prediction = game_data.ball_prediction
    car = game_data.my_car
    box_location = car.location if box_location is None else box_location
    car_rot = rotation_to_matrix([0, car.rotation[1], car.rotation[2]])
    ...
    # just_right = location_slices[:, 2] > ball.radius + hitbox_height and location_slices[:, 2] < 300
    just_right = (location_slices[:, 2] > (ball.radius + hitbox_height)) & (location_slices[:, 2] < 300)
    ...
    reachable = (state_at_time_vectorized(time_slices, velocity, boost)[0] > distance_slices) & just_right
