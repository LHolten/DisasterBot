import numpy as np
from rlbot.utils.structures.game_data_struct import MAX_NAME_LENGTH, ScoreInfo, BoxShape, \
    CollisionShape, DropShotInfo, BoostPadState, TileInfo, TeamInfo, GameInfo, \
    MAX_PLAYERS, MAX_BOOSTS, MAX_TILES, MAX_TEAMS, MAX_GOALS
from rlbot.utils.structures.ball_prediction_struct import MAX_SLICES

dtype_Vector3 = np.dtype(('<f4', 3))

# game tick packet
dtype_Physics = np.dtype([('location', dtype_Vector3),
...
dtype_Touch = np.dtype([('player_name', dtype_Name),
                        ('time_seconds', '<f4'),
...
dtype_BallInfo = np.dtype([('physics', dtype_Physics),
...
dtype_GameTickPacket = np.dtype([('game_cars', dtype_PlayerInfo * MAX_PLAYERS),
                                 ('num_cars', '<i4'),
...
# field info
dtype_BoostPad = np.dtype([('location', dtype_Vector3),
                           ('is_full_boost', '?')])
...
dtype_FieldInfoPacket = np.dtype([('boost_pads', dtype_BoostPad * MAX_BOOSTS),
                                  ('num_boosts', '<i4'),
                                  ('goals', dtype_GoalInfo * MAX_GOALS),
                                  ('num_goals', '<i4')])

# ball prediction
dtype_Slice = np.dtype([('physics', dtype_Physics),
                        ('game_seconds', '<f4')])

dtype_BallPrediction = np.dtype([('slices', dtype_Slice * MAX_SLICES),
                                 ('num_slices', '<i4')])
